...
| 备份脚本 | `deploy/backup/*.sh` + Windows `mysql-drill-windows.ps1` + `test-safety-guards.*` | 拒绝空目标/危险路径/生产库恢复；演示库可恢复到含 tmp/restore 的临时库 | 不把教学库演练写成生产容灾；禁止恢复进 `pharmacy_delivery` |
...
| 参考与核验 | 代码级核验点 | 采用的不变量 | 明确拒绝 / 许可证结论 |
|---|---|---|---|
| 本仓库 E1 `frontend/e2e/fixtures/e1_seed.sql` | `database/demo-seed.sql`：BCrypt 与 E1 同源；可售批次 QUALIFIED + 未来效期；`medicine.stock` 与可售可用量对齐 | 种子不进 Flyway；独立库 `pharmacy_delivery_demo`；禁止 DROP `pharmacy_delivery` | 不用真实患者/药店进销存；通用名仅为教学目录 |
