...
public class AdminOrderController {
    ...
    @PostMapping("/{orderId}/accept") public ApiResponse<Void> accept(@PathVariable Long orderId,@Valid @RequestBody RemarkRequest r){service.accept(CurrentUser.id(),orderId,r);return ApiResponse.success("已接单，等待打包",null);}
    @PostMapping("/{orderId}/pack") public ApiResponse<Void> pack(@PathVariable Long orderId,@Valid @RequestBody RemarkRequest r){service.pack(CurrentUser.id(),orderId,r);return ApiResponse.success("已完成打包，等待派送",null);}
    @PostMapping("/{orderId}/dispatch") public ApiResponse<Void> dispatch(@PathVariable Long orderId,@Valid @RequestBody DispatchRequest r){service.dispatch(CurrentUser.id(),orderId,r);return ApiResponse.success("已派单，订单配送中",null);}
    @PostMapping("/{orderId}/complete") public ApiResponse<Void> complete(@PathVariable Long orderId,@Valid @RequestBody RemarkRequest r){service.complete(CurrentUser.id(),orderId,r);return ApiResponse.success("订单已完成",null);}
    @PostMapping("/{orderId}/cancel") public ApiResponse<Void> cancel(@PathVariable Long orderId,@Valid @RequestBody ReasonRequest r){service.adminCancel(CurrentUser.id(),orderId,r);return ApiResponse.success("订单已取消，库存已释放",null);}
}
